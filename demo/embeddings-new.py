...
def compare_embeddings_simple(k: int, path_x:str, path_y: str) -> float:
    ...
    Y = np.array(named_embeddings_y.values())
...
def generate_llm_TSNE(
    ...
    named_embeddings = dict(zip(definitions.keys(), embeddings))
...
def generate_embedding_sample(output_path: str, path: str, n: int) -> None:
    ...
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    embeddings_sample = dict(random.sample(list(data.items()), n))
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(embeddings_sample, f, indent=2)
...
def load_embeddings_n(
    ...
    for i in range(n):
        ...
        embeddings.append(list(named_embeddings.values()))
    return embeddings
...
def dump_summary_embeddings_clap(transformer):
    ...
    with open("data/function-summaries.json", "r", encoding="utf-8") as f:
    ...
    with open("summary-embeddings-raw-clap.json", "w", encoding="utf-8") as f:
